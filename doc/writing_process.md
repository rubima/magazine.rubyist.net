...
- 地域Ruby会議レポートのタイトル形式は`RegionalRubyKaigi レポート (NUM) 〜〜`としてください。`(NUM)`はプレースホルダで、ビルド時に`created_on`順で自動採番されます。
  - 複数の地域Ruby会議レポートが並行して進行している場合に、番号が重複したり飛んだりするのを防ぐためのものです。
- 過去記事へのリンクは`{{base}}{% post_url articles/号数/日付-記事名 %}`をURLとして指定してください。
  - リンクテキストに参照先の記事タイトルを使う場合は、`{% post_title articles/号数/日付-記事名 %}`を使うと、参照先の記事タイトルの変更に自動追従します。地域Ruby会議レポートの番号のように、ビルド時に変わる可能性のあるタイトルを参照するときに有用です。
