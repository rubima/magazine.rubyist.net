---
layout: post
title: Rails Girls Kagoshima 2nd 開催レポート
short_title: Rails Girls Kagoshima 2nd
tags: 0063 RailsGirlsReport
post_author: はやぴ
created_on: 2023-09-15
---
{% include base.html %}

## はじめに

この記事は、2023年9月1日(金)、2日(土) に [GMOペパボ株式会社](https://pepabo.com/) 鹿児島オフィス にて開催された [Rails Girls Kagoshima 2nd](https://railsgirls.com/kagoshima-2023.html) のレポートです。 記事の内容は、オーガナイザーのはやぴが開催にあたり、考えたこと・感じたことを中心にまとめています。

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/gather-all.png" width="1000px">


## Rails Girls とは

[Rails Girls](https://railsgirls.com/) は、より多くの女性がプログラミングに親しみ、アイデアを形にできる技術を身につける手助けをするコミュニティで、[ガイド](https://railsgirls.jp/)を使ってプログラミングを体験できるワークショップを開催しています。 日本では、2012 年 9 月に [Tokyo](https://railsgirls.com/tokyo-2012-09-07.html) で初めて開催されました。

Rails Girls のワークショップは、プログラミング未経験の方を対象に、環境構築から Rails アプリの作成・公開までを行います。 参加者の中には PC に慣れていない方もいるため、さまざまな躓きポイントをコーチがサポートするには、オフラインで開催するのが一番です。 そのため今回も会場をお借りしてオフラインで開催しました。

## Rails Girls Kagoshima 2nd
2022年6月に[Rails Girls Kagoshima 1st](https://railsgirls.com/kagoshima-2022.html)を行いました。Rails Girlsのイベントを鹿児島で開催したのは初めてで不安もありましたが、参加者やコーチ、スポンサーなどの協力もありながら成功させることができました。Rails Girls Kagoshima 1stが地元のテレビ局に取り上げられたりと大変好評であったため、今後も継続してイベントを開催したいと思い運営メンバーを集めて企画を始めました。

## ワークショップの内容

鹿児島名物であるかき氷の「しろくま」とRubyをコンセプトに、イラストレーターの[もにやま](https://www.instagram.com/_cobone/)さんにロゴを作成していただきました。また、年間スポンサーの方々から数多くのノベルティをいただきました。心より御礼申し上げます。

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/logos.png" width="1000px">

ワークショップでは、コーチ1名参加者2名のテーブルを6席準備し、インストールからWebアプリケーションのデプロイまでサポートを行いました。

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/workshop.png" width="1000px">

開発の合間にお菓子で休憩、ベイクチーズタルトとプレスバターサンドを準備しました！

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/desserts.png" width="1000px">

ランチタイムで休憩、和やかな雰囲気で食事を楽しんでいます！

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/lunch.png" width="1000px">

スポンサーLTでは、ピクシブ株式会社 様、TokyoDev株式会社 様、株式会社現場サポート 様、個人スポンサーのyoshikouki 様にLTを行ってもらいました。

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/lt1.png" width="1000px">

また、有志LTとしてコーチと参加者も飛び入りで発表を行いました！

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/lt2.png" width="1000px">

無事にイベントが集合し、参加者とコーチ・運営で記念撮影！

<img src="{{base}}{{site.baseurl}}/images/0063-RailsGirlsKagoshima2ndReport/gather.png" width="1000px">


## おわりに

2回目のRails Girls Kagoshima、無事に大成功のイベントを終えることができました！参加者や関係者の皆様、本当にありがとうございました！
後日アンケートを取ったところ、「参加者、コーチ、運営の方々等皆さんが優しく明るく接してくれたため、楽しく参加することが出来た」「たくさん交流ができてアットホームな雰囲気で楽しかった」「頂いた資料や教科書など見て勉強してみようと思う」等の嬉しい感想をいただきました。次回以降も参加者が楽しみながら学べるイベントにできるように取り組んで参ります。
今回ご参加いただいた方の大半はプログラムを書いたことがない方でしたが、今回のワークショップを通してプログラミングに興味を持ってもらい、学習を行ったり別のエンジニアイベントに参加してみるきっかけになるとありがたいです。

## 書いた人

はやぴ [@hayapi_ppb](https://twitter.com/hayapi_ppb) GMOペパボ株式会社 CTO室鹿児島エンジニアリングチーム サブマネージャー
