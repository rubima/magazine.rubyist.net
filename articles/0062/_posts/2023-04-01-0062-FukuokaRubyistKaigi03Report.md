---
layout: post
title: RegionalRubyKaigi レポート (xx) 福岡Rubyist会議03
short_title: RegionalRubyKaigi レポート (xx) 福岡Rubyist会議03
...
## RegionalRubyKaigiレポート (xx) 福岡Rubyist会議02
...
YassLab (株)の安川さんによる講演です。実際にiPadで今回の講演を生配信を行いながら、配信や動画作成をカンタンに行う方法を講演されました。
...
ANDPADの柴田さんによる講演です。
はじめに各種リリース用語(バックポート・パッケージ・パッケージング・リリース)の解説が行なわれました。次に、ドキュメントの粒度やリリース体制・リリースサイクルなどをRubyと他のプログラミング言語の比較を説明されました。
...
また、この講演の質疑応答では会場のRubyistたちから、Rubyist Magazineへの熱い思いが多数出ていました。
