...
: actual が Proc である場合に、Proc で実行される手続きが expected で指定された Symbol を throw することを期待します。expected を指定しない場合は、何らかの Symbol が throw されることを期待します。
...
| rake spec:models           |  spec/models ディレクトリ以下にあるモデルの spec を実行します。|
