...
2025年9月6日、岐阜県岐阜市の長良川うかいミュージアムにて、ながらRuby会議01を開催しました。岐阜市で毎月開催されている[nagara.rb](https://nagara-rb.connpass.com/)としては初めての地域Ruby会議で、当日は約40名の参加者（スタッフ含め60名弱）が集まりました。
...
nagara.rb は清流・長良川のほとりで、言語を問わずもくもくとコーディングを楽しむコミュニティです。月1回のもくもく会を続けてきて、このながらRuby会議01をもって **100回目**の節目を迎えました。「nagara.rbが今回で100回目を迎えました！」という発表があると、会場から大きな拍手が沸き起こりました。
...
Rubyでインタプリタを自作した経験をもとに、字句解析・構文解析・評価といった処理の流れを解説されました。
...
組み込み向けの軽量なRuby実装「PicoRuby」を使い、LEDを光らせるところから始めて、センサーや音声認識まで広げていく過程を紹介されました。
...
RuboCopコミッターとして毎日OSSに関わり続けているKoichi ITOさんが、開発環境の整え方や継続のコツを共有いただきました。
...
興味深かったのは、Forkしたリポジトリを常に `upstream` に設定し、Fork 元を `origin` にするという運用です。こうしておけば、コミット権のあるリポジトリでもないリポジトリでも、`git push upstream head` という同じコマンドで済みます。
...
最初のスポンサーセッションは、株式会社スタメン様。エンゲージメントプラットフォーム「TUNAG」のアーキテクチャについて、モノリス構成から新機能をサブサービスとして切り出していく取り組みを紹介していただきました。認可サービスを介したSSO連携を維持しながら「TUNAGチャット」を分離した事例は、実践的な知見として参考になりました。

### 株式会社SmartHR

![]({{base}}{{site.baseurl}}/images/0066-NagaraRubyKaigi01Report/image09.jpg)

2つ目のスポンサーセッションは、株式会社SmartHR様。技術顧問のwillnetさんが登壇し、カンファレンスのプロポーザルに応募したものの惜しくも落選したテーマを5分に凝縮して発表してくださいました。Rails拡張gemの設計にまつわる内容で、「落選ネタを供養する場」としてのスポンサーセッション活用が微笑ましく、会場も盛り上がりました。

### 株式会社Leaner Technologies

![]({{base}}{{site.baseurl}}/images/0066-NagaraRubyKaigi01Report/image10.jpg)

最後のスポンサーセッションは、株式会社Leaner Technologies様。「マネジメントは必要だが、マネージャーは必ずしも必要ではない」という切り口から、役割と責務のあり方を問い直す内容でした。エンジニア組織のマネジメントに興味のある参加者にとって、考えるきっかけを与えてくれるセッションでした。

## 日本Rubyの会の紹介

![]({{base}}{{site.baseurl}}/images/0066-NagaraRubyKaigi01Report/image11.jpg)

日本Rubyの会の理事である島田さんによる、同会の紹介がありました。組織の活動内容や地域Ruby会議への支援の仕組みについて説明があり、ながらRuby会議01もその支援のもとで開催されたことが改めて紹介されました。
...
懇親会では、地元岐阜のお菓子と日本酒、各種ドリンクで盛り上がりました。
...
懇親会の後には、アフターイベントとして[長良川鵜飼](https://www.ukai-gifucity.jp/)を観覧しました。
...
- **湊（[@ueferi](https://github.com/ueferi)）**
  - ながらRuby会議01実行委員。nagara.rbメンバー。
  - X: [@ueferi](https://x.com/ueferi)
